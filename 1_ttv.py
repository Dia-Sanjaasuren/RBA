...
try:
    ...
    for index in business_unit_order:
        if index in pivot_table.index:
            ...
            for column in payment_method_order:
                ...
                dollar_row[f"{column}\nGTV"] = format_currency(value)
            
            # Add row total
            dollar_row["Total\nGTV"] = format_currency(row_total)
            
            # Calculate % of total TTV
            if index != 'Total':
                pct_of_total = (row_total / grand_total * 100) if grand_total > 0 else 0
                dollar_row["% Of Total TTV\n%"] = f"{pct_of_total:.2f}%"
            else:
                dollar_row["% Of Total TTV\n%"] = "100.00%"
    ...
    final_table_display.loc[final_table_display['Type'] == '% of TTV', 'Business Unit'] = '% of TTV'
    ...
    gb.configure_column("Type", width=80, hide=True)  # Hide the empty column name from MultiIndex
    ...
    gb.configure_column("% Of Total TTV\n%", width=100)
    ...
    with col3:
        st.metric("Total TTV", format_currency(df['TOTAL_TTV'].sum()))
